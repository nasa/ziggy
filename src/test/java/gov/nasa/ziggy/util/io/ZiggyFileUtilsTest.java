 ...
 * @author Bill Wohler
 */
public class ZiggyFileUtilsTest {
    ...
    public void testRegularFilesInDirTree() throws IOException {
        ...
        Map<Path, Path> regularFiles = ZiggyFileUtils.regularFilesInDirTree(testDirPath);
    ...
    public void testModeToPosix() {
        assertEquals("r--------", ZiggyFileUtils.modeToPosixFileString(100));
        assertEquals("-w-------", ZiggyFileUtils.modeToPosixFileString(200));
        assertEquals("rw-------", ZiggyFileUtils.modeToPosixFileString(300));
        assertEquals("--x------", ZiggyFileUtils.modeToPosixFileString(400));
        assertEquals("r-x------", ZiggyFileUtils.modeToPosixFileString(500));
        assertEquals("-wx------", ZiggyFileUtils.modeToPosixFileString(600));
        assertEquals("rwx------", ZiggyFileUtils.modeToPosixFileString(700));

        assertEquals("---r-----", ZiggyFileUtils.modeToPosixFileString(10));
        assertEquals("------r--", ZiggyFileUtils.modeToPosixFileString(1));
        assertEquals("---------", ZiggyFileUtils.modeToPosixFileString(0));

        assertEquals("rwxr----x", ZiggyFileUtils.modeToPosixFileString(714));
    }
    ...
    public void testSetPosixPermissionsRecursively() throws IOException {
        ...
        ZiggyFileUtils.setPosixPermissionsRecursively(testDir.toPath(), "r--------", "r-x------");
        assertEquals("r-x------",
            PosixFilePermissions.toString(Files.getPosixFilePermissions(testDir.toPath())));
        assertEquals("r-x------",
    ...
    public void testOverwritePermissions() throws IOException {
        ...
        ZiggyFileUtils.writeProtectDirectoryTree(testDir.toPath());
        assertNotEquals(ZiggyFileUtils.DIR_OVERWRITE_PERMISSIONS,
            PosixFilePermissions.toString(Files.getPosixFilePermissions(testDir.toPath())));

        ZiggyFileUtils.prepareDirectoryTreeForOverwrites(testDir.toPath());
        assertEquals(ZiggyFileUtils.DIR_OVERWRITE_PERMISSIONS,
            PosixFilePermissions.toString(Files.getPosixFilePermissions(testDir.toPath())));
        assertEquals(ZiggyFileUtils.DIR_OVERWRITE_PERMISSIONS,
            PosixFilePermissions.toString(Files.getPosixFilePermissions(testSubdir.toPath())));
        assertEquals(ZiggyFileUtils.FILE_OVERWRITE_PERMISSIONS,
            PosixFilePermissions.toString(Files.getPosixFilePermissions(testRegularFile.toPath())));
        assertEquals(ZiggyFileUtils.FILE_OVERWRITE_PERMISSIONS, PosixFilePermissions
            .toString(Files.getPosixFilePermissions(testSubdirRegularFile.toPath())));
    }
    ...
    public void testReadOnlyPermissions() throws IOException {
        ...
        ZiggyFileUtils.prepareDirectoryTreeForOverwrites(testDir.toPath());
        assertNotEquals(ZiggyFileUtils.DIR_READONLY_PERMISSIONS,
            PosixFilePermissions.toString(Files.getPosixFilePermissions(testDir.toPath())));

        ZiggyFileUtils.writeProtectDirectoryTree(testDir.toPath());
        assertEquals(ZiggyFileUtils.DIR_READONLY_PERMISSIONS,
            PosixFilePermissions.toString(Files.getPosixFilePermissions(testDir.toPath())));
        assertEquals(ZiggyFileUtils.DIR_READONLY_PERMISSIONS,
            PosixFilePermissions.toString(Files.getPosixFilePermissions(testSubdir.toPath())));
        assertEquals(ZiggyFileUtils.FILE_READONLY_PERMISSIONS,
            PosixFilePermissions.toString(Files.getPosixFilePermissions(testRegularFile.toPath())));
        assertEquals(ZiggyFileUtils.FILE_READONLY_PERMISSIONS, PosixFilePermissions
            .toString(Files.getPosixFilePermissions(testSubdirRegularFile.toPath())));
    }
    ...
    public void testCleanDirectoryTree() throws IOException {
        ...
        ZiggyFileUtils.cleanDirectoryTree(testSubdir.toPath(), true);
    ...
    public void testUnforcedCleanOfWriteProtectedDirTree() throws IOException {
        ...
        // Clean the testSubdir
        ZiggyFileUtils.cleanDirectoryTree(testSubdir.toPath());
    }

    @Test
    public void testDeleteNonexistentDirectory() throws IOException {
        ZiggyFileUtils.deleteDirectoryTree(testDir.toPath().resolve("non-existent-dir"));
    }
