...
public class MessageHandlersForTest {
	...
	/**
	 * Subclass of {@link ClusterController} that won't bomb when trying to check
	 * the state of the (nonexistent) worker.
	 *
	 * @author PT
	 */
	...
	public static class ClientSideMessageHandlerForTest extends MessageHandler {
	    ...
	    public ClientSideMessageHandlerForTest() {
	        super(new PigMessageDispatcher(null, null, false));
	    }
	...
	/**
	 * Subclass of {@link PigMessageDispatcher} that doesn't try to shut down the
	 * console (because it's not actually running in the console).
	 *
	 * @author PT
	 *
	 */
	...
	/**
	 * Performs message handling in unit tests for messages sent to the server
	 * (worker). It stores all {@link MessageFromClient} instances in a Set and
	 * ignores all other kinds of messages. Once a specified number of messages have
	 * been received, it replies with a {@link MessageFromServer}.
	 *
	 * @author PT
	 */
