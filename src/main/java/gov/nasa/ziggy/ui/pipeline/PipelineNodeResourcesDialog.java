...
/**
 * Dialog box for editing the worker count and heap size for a {@link PipelineNode}.
 * <p>
 * The dialog box is initialized from the state of the {@link PipelineNode}: if the node uses
 * default values for worker count or for heap size, that information is captured, the initial
...
public class PipelineNodeResourcesDialog extends JDialog {
    ...
    private JPanel createDataPanel() {
        ...
        JLabel pipelineText = new JLabel(pipelineName);
    ...
    /**
     * Capture the current state of worker resource parameters in the relevant {@link PipelineNode}
     * instance and close the dialog box. Resources are only captured in cases where the relevant
     * "use default" dialog box is not checked (i.e., places where the user wants default values are
     * set to null).
     */
     ...
     * field for the worker count is disabled and the max worker count on the {@link PipelineNode}
     * is set to zero, indicating that the default value should be used; the user current value for
     * the worker count is captured if the text field is valid.
     */
     ...
     * disabled to enabled, the {@link PipelineNode} value of the heap size is set to a value that
     * indicates that the default should be used, the text field and the unit buttons are disabled,
     * and the current value of the text box is captured if it is valid. If the box goes from
     * disabled to enabled, the text field and unit buttons are enabled and the current user value
     * of the heap is entered into the text field.
     */
