...
public class SubtaskMaster implements Runnable {
            ...
            try {
                if (getWriteLockWithoutBlocking(lockFile)) {
                    ...
                    if (!checkSubtaskState(subtaskDir)) {
                        executeSubtask(subtaskDir, threadNumber);
                    }
    ...
    private void executeSubtask(File subtaskDir, int threadNumber) {
        ...
        try {
            ...
            SubtaskExecutor subtaskExecutor = subtaskExecutor();
    ...
    void logAlgorithmStackTrace(File subtaskDir, int subtaskIndex) {
        File errorFile = AlgorithmInterfaceUtils.errorFile(subtaskDir, binaryName);
        if (errorFile.exists()) {
            AlgorithmErrorReturn algorithmStack = new AlgorithmErrorReturn();
            new Hdf5AlgorithmInterface().readFile(errorFile, algorithmStack, true);
    ...
    SubtaskExecutor subtaskExecutor() {
        return SubtaskExecutorFactory.newInstance(this);
    }
