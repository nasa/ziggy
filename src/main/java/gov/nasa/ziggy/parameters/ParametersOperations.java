...
public class ParametersOperations {
    ...
    public List<ParameterSetDescriptor> exportParameterLibrary(String destinationPath,
        ...
        for (ParameterSet paramSet : parameters) {
            ...
            Parameters parametersInstance = paramSet.parametersInstance();
            String className = "";
            try {
                className = parametersInstance.getClass().getName();
            ...
            ParameterSetDescriptor parameterSetDescriptor = new ParameterSetDescriptor(name,
                className);
            parameterSetDescriptor.setParameterSet(paramSet);
    ...
    private void createNewParameterSet(ParameterSetDescriptor desc, ParamIoMode ioMode) {
        ...
        if (ioMode == ParamIoMode.STANDARD) {
            ...
            newParamSet.setTypedParameters(desc.getImportedProperties());
            newParamSet.setClassname(desc.getClassName());
            new ParameterSetCrud().merge(newParamSet);
        }
    }
    ...
    private void overrideExistingParameters(ParameterSetDescriptor desc,
        ...
        mergedParameterSet.setTypedParameters(databaseParameterSet.copyOfTypedParameters());
        ...
        Map<String, TypedParameter> mergedProperties = nameToTypedPropertyMap(
            mergedParameterSet.getTypedParameters());
        ...
        desc.setState(updateParameters(databaseParameterSet,
            mergedParameterSet.parametersInstance(), ioMode));
    }
     ...
     * <p>
     * For instances of {@link Parameters}, the replacement allows not only the values but also the
     * parameters themselves. The original parameters (names, types, and values) are replaced. For
     * all other classes, the current parameters of the class are used (which may differ from the
     * values in the library if the class has been modified). In either case, the current library
     * parameters are not copied to the new parameter set. For partial parameter set replacement
     * (i.e., keep some values and replace others), use parameter overrides.
     */
    private void replaceExistingParameters(ParameterSetDescriptor desc,
        ...
        desc.setState(updateParameters(databaseParameterSet, desc.parametersInstance(), ioMode));
    }
    ...
    private ParameterSetDescriptor.State updateParameters(ParameterSet databaseParameterSet,
        Parameters newParameters, ParamIoMode ioMode) {
        // If the parameter values have changed, commit the new parameters to the database
        PipelineOperations pipelineOps = new PipelineOperations();
        if (pipelineOps.compareParameters(databaseParameterSet.parametersInstance(),
            newParameters)) {
        ...
        if (ioMode == ParamIoMode.STANDARD) {
            pipelineOps.updateParameterSet(databaseParameterSet, newParameters, false);
        }
        ...
        if (ioMode != ParamIoMode.NODB) {
            ...
            for (ParameterSet libraryParamSet : allLibraryEntries) {
                ...
                if (!importNames.contains(name)) {
                    Parameters libraryParameters = libraryParamSet.parametersInstance();
                    String className = "";
                    try {
                        className = libraryParameters.getClass().getName();
                    } catch (PipelineException e) {
                        continue;
                    }
                    ...
                    newDesc.setLibraryProps(formatProps(libraryParameters.getParameters()));
                    entries.add(newDesc);
                }
            }
        }
