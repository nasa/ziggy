 ...
 * <li>Members that are DefaultParameters instances require the name management but also need to
 * translate from the name-value-type TypedProperty instances stored in the DefaultParameters
 * instance to HDF5 groups and datasets formatted the way that the end-users expect them.
 * </ol>
...
public class ModuleParametersHdf5Array extends AbstractHdf5Array {
     ...
     * <p>
     * DefaultParameters instances are written using the parameter set name as the group name, with
    ...
    public List<Long> write(long fieldGroupId, String fieldName) {
        ...
        for (Parameters parametersInstance : parametersObject.getModuleParameters()) {
            ...
            if (!(parametersInstance instanceof DefaultParameters)) {
                ...
                // DefaultParameters:
                List<Long> defaultParametersSubGroups = writeDefaultParametersInstance(fieldGroupId,
                    (DefaultParameters) parametersInstance);
                parametersInstanceGroupId = defaultParametersSubGroups.get(0);
                subGroupIds.addAll(defaultParametersSubGroups);
    ...
    /**
     * Write an instance of DefaultParameters to HDF5. The instance is written in a way that will
     * appear to an algorithm user as identical in structure to the way that a classed Parameters
     * instance does (i.e., a collection of array groups, with 1 parameter per group).
     */
    private List<Long> writeDefaultParametersInstance(long fieldGroupId,
        DefaultParameters defaultParameters) {
        List<Long> subGroupIds = new ArrayList<>();
        String parameterSetName = defaultParameters.getName();
        ...
        for (TypedParameter parameter : defaultParameters.getParameters()) {
            subGroupIds.addAll(
    ...
    public void read(long fieldGroupId) {
        ...
        List<Parameters> parameterSets = new ArrayList<>();
        ...
        for (String parameterSetName : parameterSetNames) {
            ...
            // check for an original parameter set name, which indicates a DefaultParameters
            // instance
            if (!H5.H5Aexists(parameterSetGroupId,
                ...
                // DefaultParameters instances are more complicated
                DefaultParameters defaultParameters = new DefaultParameters();
                Set<TypedParameter> parameters = new HashSet<>();
                defaultParameters.setName(readStringAttribute(parameterSetGroupId,
                    Hdf5ModuleInterface.PARAMETER_SET_ORIGINAL_NAME_ATT_NAME));
                ...
                for (String parameterName : parameterNames) {
                    ...
                    PrimitiveHdf5Array primitiveHdf5Array = PrimitiveHdf5Array
                        .forReadingDefaultParameters(parameterGroupId, parameterName);
                    ...
                    parameters.add(toTypedParameter(javaArray, parameterName, scalar));
                }
                defaultParameters.setParameters(parameters);
                parameterSets.add(defaultParameters);
