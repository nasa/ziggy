...
## Changing Algorithm Parameter Values

Algorithm parameters can be changed to affect how the algorithm behaves. To see how this works, select the `Parameter Library` content menu item. You'll see this:

<img src="images/parameter-library.png" style="width:32cm;"/>

As you can see, the parameter set defined in `sample-pipeline.xml` is represented here. What else does the table tell us?
...
The parameters that were defined as booleans in `sample-pipeline.xml` have check boxes you can check or uncheck. The other parameter types mostly behave the way you expect, but the array types offer some additional capabilities. If you click the `dummy array parameter` parameter, it will change thusly:
