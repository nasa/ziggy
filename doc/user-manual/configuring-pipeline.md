...
- [Python Pipeline Algorithms](python-pipeline-algorithms.md) talks about how to wire pipeline algorithms written in Python into Ziggy.
- [Other Pipeline Algorithms](other-pipeline-algorithms.md) talks about how to wire pipeline algorithms written in arbitrary languages into Ziggy.
...
In real life, you would want the working properties file to be outside of the directories managed by the version control system. This allows you to modify the file without fear that you will accidentally push your changes back to the repository's origin! We suggest that you start by copying the [pipeline.properties.EXAMPLE file](../../etc/pipeline.properties.EXAMPLE) to someplace outside of the Git-controlled directories, rename it, and modify it so that it suits your need. This file expects that you've defined the environment variable `ZIGGY_HOME` that points to your Ziggy installation, which is currently the `build` directory within the ziggy directory that was created when you compiled Ziggy.
